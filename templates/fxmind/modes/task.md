...
| **Plan-first / analyze-only** | ambiguous scope, irreversible/outward action, user asked for a plan, or **analyze/analisar/review/investigar/diagnosticar/propor/planear** without apply/fix/implement | Findings + proposed plan. **Stop for AskQuestion approval**. Edit only after explicit approval. "Analisa e corrige" = full Task after a short analysis summary. |

Tie-breaks: (1) any plan-first / analyze-only signal beats task; (2) mixed "why + fix" = task whose report also answers the question; (3) unsure task vs plan-first → plan-first / analyze-only.
...
- **CLASS:** `task` | `plan-first` | `analyze-only` | (if you mis-routed a question, stop and do not edit)
- **Goal**
...
Output:
```
🛑 GATE A COMPLETE — CLASS: <task|plan-first|analyze-only>, GOAL: <one-line>, DONE: <criterion + verify>, SCOPE: <files>, TOPICS: <list>, RISKS: <list or none>
```
Then call **`fxmind_record_gate`** with `gate: "A"`.

**Plan-first / analyze-only:** after Gate A (and Gate B if needed for evidence), deliver the plan/findings and **STOP** for AskQuestion — no Implement until the user approves.
...
3. **Smallest correct change**; match existing style; prefer existing helpers/events/framework APIs.
