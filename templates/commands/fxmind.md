...
1. Read `.fxmind/knowledge-graph.json` (fallback: extract `GRAPH_DATA` from `knowledge-graph.html`).
...
1. Read `.fxmind/memory/_index.md` first if it exists.
...
5. Use `.fxmind/memory.template.md` structure, compact English, `lang: en-compact`, ~25–60 lines.
...
- `Memória criada: .fxmind/memory/<topic>.md`
- or `Memória atualizada: .fxmind/memory/<topic>.md`
...
Save to **`.fxmind/audit-<resource-name>.md`**.
...
- Path to full report: `.fxmind/audit-<name>.md`
...
- Path: `.fxmind/memory/<topic>.md`
- If codebase changed heavily since last learn → suggest re-running `/fxmind learn <topic>`
- Suggest `/fxmind graph` to refresh the 3D knowledge map
...
Save **`.fxmind/memory-health.md`** using `memory-health.template.md`:
...
- Path: `.fxmind/memory-health.md`
- Auto-fixed topics (fix mode)
- Topics needing `/fxmind learn <topic>` (manual)
- Suggest `/fxmind graph` if memories changed
...
- `.fxmind/memory/_index.md` (topic table)
- `.fxmind/memory/*.md` (exclude `_index.md`)
- `.fxmind/topic-catalog.md` (catalog orphans)

If `.fxmind/` is missing → user must run fxmind installer first.

Legacy fallback: if `.fxmind/memory/` is empty but `.cursor/fivem/memory/`, `.gemini/fivem/memory/`, or `.opencode/fivem/memory/` has topics, read from legacy paths and suggest `fxmind -y` to migrate.
...
| `group` | `"learned"` |
| `file` | path relative to project root (e.g. `.fxmind/memory/craft.md`) |
...
Always use `"agent": "shared"` and `"fxmindDir": ".fxmind"` — the graph is shared across all agents.

### Step 3 — Write JSON and HTML

Write the same JSON object to **both**:

1. `.fxmind/knowledge-graph.json` (2-space indent, valid JSON)
2. `.fxmind/knowledge-graph.html` — replace only the graph data payload

Read `.fxmind/knowledge-graph.html`. Replace **only** the graph data payload with the JSON from step 2 (2-space indent, valid JavaScript). Do not change any other part of the file.
...
Write the result back to `.fxmind/knowledge-graph.html`.
...
Remind user: re-run `/fxmind graph` after `/fxmind learn`; use `/fxmind query` for agent retrieval.
...
If graph lacks enough information → say so; suggest `/fxmind learn <topic>` or `/fxmind graph`.
