...
- **N3 Big data in pieces.** Lists that grow with players/items, or payloads above ~8–16 KB: the client pulls pages on demand (when the UI opens, per page/tab); bulk server→client delivery goes through cerberus `SendFullSync`/`SendDeltaSync` when the project has it, otherwise `TriggerLatentClientEvent(name, target, bps, ...)` or chunks split **once** from the view cache and reused for every player, with `Wait` between chunks (`performance.md` §2.2.1). Never one giant event, never re-building or re-chunking the payload per player.
...
- **N7 Seed by server push.** Data every client needs (points, blips, zones, garages): one server `CreateThread` at resource start loads the DB into the cache, builds the client view **once** and sends it to players online (`-1` — covers `ensure`; on boot nobody is online, so it only builds); the player-loaded hook (`playerConnect`, `QBCore:Server:PlayerLoaded`, `esx:playerLoaded`) sends that same cached view to `source`; CRUD patches only the edited key and sends a delta. The client never asks for its initial data (`CreateThread` + `Wait` + `TriggerServerEvent("*:requestSync")` is forbidden), and no handler rebuilds the view or reloads the whole table per player.
...
- **D1 No DB in hot paths.** Never query or write the DB inside loops, ticks, frequent events, per-player iterations or repeatable NUI callbacks. Shared world data loads once at resource start (N7); per-player data loads on join/first use into a cache (`cacheaside` when installed; invalidate in the mutating action); write-behind: mark dirty, flush in batches on interval, `playerDropped` and `onResourceStop`.
