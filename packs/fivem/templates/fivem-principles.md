...
Short rules with IDs. Cite the IDs in Gate A/B notes and in the Gate V `review` ("N1 N2 D1 ok; N3 n/a"). Details and examples live in `.fxmind/skills/fivem-development/`; this file decides.
...
- **N3 Big data in pieces.** Lists that grow with players/items, or payloads above ~16 KB: the client pulls pages on demand (when the UI opens, per page/tab), or the server streams chunks with `TriggerLatentClientEvent(name, target, bps, ...)`. Never one giant event.
...
- **N5 Sync through the client.** Before adding a server event to share one player's/entity's data with others, check whether clients can read it themselves: `Entity(ent).state` (replicates only to clients that have the entity in scope), `Player(serverId).state` (replicates to everyone — keep tiny), natives (coords, vehicle, health, animation). React with `AddStateBagChangeHandler`, not polling. `GlobalState` only for tiny, rarely changing global flags. Server-only values: `state:set(key, value, false)`.
- **N6 Anti-flood.** Every client→server event and callback: server-side per-source cooldown + validation; the client debounces UI actions and never fires events inside frame loops. Batch repeated actions into one call.

## Server & database

- **D1 No DB in hot paths.** Never query or write the DB inside loops, ticks, frequent events, per-player iterations or repeatable NUI callbacks. Load on join/first use into a cache (cache-aside with invalidation on the mutating action); write-behind: mark dirty, flush in batches on interval, `playerDropped` and `onResourceStop`.
